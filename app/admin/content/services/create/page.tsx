...
import { createService, getServices } from "@/lib/db-service"
...
export default function CreateServicePage() {
  ...
  const [formData, setFormData] = useState<Omit<Service, "id" | "createdAt" | "updatedAt" | "createdBy">>({
    title: "",
    slug: "",
    description: "",
    shortDescription: "",
    ...
    features: [],
    status: "draft",
    order: 999, // Will be updated before saving
  })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Get the current services to determine the next order
      const services = await getServices()
      const maxOrder = services.length > 0 ? Math.max(...services.map((s) => s.order)) : 0

      // Create the service with the next order
      await createService({
        ...formData,
        order: maxOrder + 1,
      })
        ...
        <TabsContent value="content" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Enter the detailed content of your service</CardDescription>
            </CardHeader>
            <CardContent>
